...
// 새 문서가 저장될 때 탭 2 문서 목록 업데이트
document.addEventListener('documentSaved', function() {
    tab2Page = 0;
    document.getElementById('tab2-document-list').innerHTML = '';
    loadTab2Documents();
});
...
tabTriggerList.forEach(function(tabEl) {
    tabEl.addEventListener('shown.bs.tab', function (e) {
        if (e.target.id === 'tab2-tab' && tab2Page === 0) {
            loadTab2Documents();
        }
    });
});
...
// document.addEventListener('DOMContentLoaded', function () {
//     const switch1 = document.getElementById('ai-toggle');
    
//     // 스위치 상태가 변경될 때마다 chrome storage에 상태를 저장합니다.
//     switch1.addEventListener('change', function () {
//         chrome.storage.local.set({ 'tab2SwitchActive': switch1.checked });
//     });

//     // 저장된 스위치 상태를 로드하여 스위치 UI에 반영합니다.
//     chrome.storage.local.get(['tab2SwitchActive'], function (result) {
//         if (result.tab2SwitchActive) {
...
function initializeTab2SearchEvents() {
    const searchBox2 = document.querySelector('#tab2-search-input');
    ...
    // 검색 실행 함수
    function executeTab2Search() {
        const queryText = searchBox2.value.trim();
        if (queryText) {
            searchTab2Documents(queryText);
            searchBox2.value = ''; // 검색 후 검색창 비우기
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
    // 탭 2의 검색 버튼 및 검색 창 관련 이벤트 추가
    const searchBox2 = document.querySelector('#tab2-content .search-group .form-control');
    const searchButton2 = document.getElementById('searchButton2');

    if (searchBox2 && searchButton2) {
        // Enter 키 입력 시 검색 실행
        searchBox2.addEventListener('keydown', function (event) {
            if (event.key === 'Enter') {
                event.preventDefault(); // 기본 동작 방지 (엔터로 줄바꿈되는 것을 막음)
                executeSearchTab2(); // 검색 실행 함수 호출
            }
        });
...
// 검색 기능 구현
function searchTab2Documents(queryText) {
...
// 검색 결과 목록 표시 함수
function createTab2DocumentItem(container, data) {
    let docItem = document.createElement('div');
    docItem.className = 'document-item';
    ...
    viewDetailsBtn.addEventListener('click', function (e) {
        e.stopPropagation();
        showDocumentDetailsModal2(data.id);
    });
...
function toggleKeywordSelection(element) {
    ...
    if (element.classList.contains('selected')) {
        ...
        fetch('/pages/search_by_tkm', {
            ...
            body: JSON.stringify({tag:'keyword', keyword: keyword }),
        })
        .then(response => response.json())
        .then(dataList => {
            // 문서 목록 업데이트
